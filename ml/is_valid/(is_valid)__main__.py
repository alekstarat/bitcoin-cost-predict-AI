...
def get_filtered_data(load_weights=True, n = 0):
  ...
  model03.add(Dense(200, activation="sigmoid"))
  ...
  if load_weights == True:
    ...
    a = model03.predict(X_train)
    print(np.argmax(a[n]))
    print(np.argmax(y_train[n]))
  else:

    model03.fit(X_train[:150], y_train[:150], epochs=100, verbose=1, validation_data=(X_train[150:], y_train[150:]))
    model03.save_weights('ml/is_valid/parser/weights.h5')
    a = model03.predict(X_train)
    print(np.argmax(a[n]))
    print(np.argmax(y_train[n]))
...
get_filtered_data(load_weights=True, n = 15)